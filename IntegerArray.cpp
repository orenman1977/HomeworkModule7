...
{
	if (_array_lenght >= 1 << 20) { throw bad_length(_array_lenght); }	// ограничение длины массива до 1Мб
	this->_my_array = new int[_array_lenght] {};
}

IntegerArray::IntegerArray(int* arr, size_t len) :_array_lenght(len), _my_array(arr), _f_index(0)
{
	if (len >= (1 << 20)) { throw bad_length(_array_lenght); }
...
{
	if (newlen >= (1 << 20)) { throw bad_length(newlen); }
...
{
	if (_array_lenght == 0 || index > _array_lenght) { throw bad_range(index); }
	++_array_lenght;
